...
y_train = train_dataset.battleneturl
X_train = pd.get_dummies(train_dataset, columns = ['race']).drop(['battleneturl', 'playername'], axis=1)
print(X.columns)
...
X_test = pd.get_dummies(test_dataset, columns = ['race'])

full_svc = svm.SVC(kernel = 'linear')
full_svc.fit(X_train, y_train)
predictions = full_svc.predict(X_test)
